...
if (sifirla === "sıfırla") {  
 ...
 if (!sifirla) {
  ...
  if (kufurE === null) kufur = `${client.emojis.get(client.emoji.kapalı)} ${p}küfür-engel aç`
  if (kufurE === "Aktif") kufur = `${client.emojis.get(client.emoji.açık)} ${p}küfür-engel kapat`
  // </> :Küfür Engel: <\> 
  // </> :Reklam Engel: <\>
  let reklamE = await db.fetch(`reklamE_${message.guild.id}`)
  let reklam;
  
  if (reklamE === null) reklam = `${client.emojis.get(client.emoji.kapalı)} ${p}reklam-engel aç`
  if (reklamE === "Aktif") reklam = `${client.emojis.get(client.emoji.açık)} ${p}reklam-engel kapat`
  // </> :Reklam Engel: <\> 
  // </> :Büyük Harf Engel: <\>
  let buyukharfE = await db.fetch(`capsE_${message.guild.id}`)
  let buyukharf;
  
  if (buyukharfE === null) buyukharf = `${client.emojis.get(client.emoji.kapalı)} ${p}büyükharf-engel aç`
  if (buyukharfE === "Aktif") buyukharf = `${client.emojis.get(client.emoji.açık)} ${p}büyükharf-engel kapat`
  // </> :Büyük Harf Engel: <\>
  // </> :Hg Bb Kanalı: <\>
